...
begin
  ...
  try
    ...
    FDM.IBDataSetFormaPagamento.Close;
    FDM.IBDataSetFormaPagamento.SelectSQL.Text:='SELECT * FROM FORMAPAGAMENTO WHERE ID = ' + intToStr(IdFP);
    FDM.IBDataSetFormaPagamento.Open;
    FDM.IBDataSetFormaPagamento.Delete;

    FDM.IBTransaction1.CommitRetaining;

    FDM.IBDataSetFormaPagamento.Close;
    FDM.IBDataSetFormaPagamento.SelectSQL.Text:='SELECT * FROM FORMAPAGAMENTO ORDER BY ID ';
    FDM.IBDataSetFormaPagamento.Open;

    FDM.IBTransaction1.CommitRetaining;
    MSG := UmaFP.FP + ' REMOVIDA COM SUCESSO.';

  except
    FDM.IBTransaction1.RollbackRetaining;
    MSG := UmaFP.FP + ' NÃO PODE SER REMOVIDA.';
  end;
