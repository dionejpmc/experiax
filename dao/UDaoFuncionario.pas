...
begin
     UmFuncionario:=TFuncionario(Objeto);
   try
   if Self.Buscar(UmFuncionario.Id,'') then
   begin
       idFuncionario:=UmFuncionario.Id;
       if not (FDM.IBTransaction1.Active) then
          FDM.IBTransaction1.StartTransaction;

       FDM.IBDataSetFuncionario.Close;
       FDM.IBDataSetFuncionario.SelectSQL.Text:='SELECT * FROM FUNCIONARIO WHERE ID = ' + intToStr(idFuncionario);
       FDM.IBDataSetFuncionario.Open;
       FDM.IBDataSetFuncionario.Delete;
       FDM.IBTransaction1.CommitRetaining;
       FDM.IBDataSetFuncionario.Close;
       FDM.IBDataSetFuncionario.SelectSQL.Text:='SELECT * FROM FUNCIONARIO ORDER BY ID ';
       FDM.IBDataSetFuncionario.Open;
...
begin
  ...
  try
     ...
     begin
         ...
         MSG:=UmFuncionario.Nome+' EDITADO COM SUCESSO.';
     end;
